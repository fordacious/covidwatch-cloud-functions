...
func generateChallenge(workFactor uint64) Challenge {
	...
	if _, err := rand.Read(nonce[:]); err != nil {
		panic(fmt.Sprint("could not generate random challenge:", err))
	}
...
// Guarded is a request which is guarded by a proof of work rate limiter. The
// type implementing Guarded can be deserialized from JSON, and one of its
// fields is a ChallengeSolution.
type Guarded interface {
	ChallengeSolution() ChallengeSolution
}

// UnmarshalAndValidateSolution unmarshals frrom ctx.HTTPRequest().Body into
// req, extracts the challenge solution, and validates it. In particular, it
// validates that:
